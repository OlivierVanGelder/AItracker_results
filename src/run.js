...
async function main() {
  ...
  const exportTimeoutMs = Number(process.env.EXPORT_TIMEOUT_MS || "120000");
  ...
  try {
    ...
    const exportButton = page.getByRole("button", { name: exportButtonText });
    await exportButton.waitFor({ timeout: exportTimeoutMs });
